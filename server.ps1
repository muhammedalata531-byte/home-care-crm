...
try {
    ...
    while ($listener.IsListening) {
        ...
        if ($rawUrl -eq "/api/latest-recording") {
            ...
            $files = Get-ChildItem -Path $recordingsPath -File | Where-Object { $_.Extension -in @(".mp3", ".wav", ".ogg", ".m4a") } | Sort-Object LastWriteTime -Descending
            ...
            if ($files -and $files.Count -gt 0) {
                ...
                $jsonObj = @{
                    success = $false
                    message = "No recordings found in Desktop/Recordings folder"
                }
            }
